...
# class Encoder(Network_V2):
#     def __init__(self, img_dim, latent_dim):
#         super().__init__(loss_name="MSE")# "CrossEntropy")

#                 # encoder
#         self.add(Dense_V2(n_units=512, input_shape=(img_dim,), activation="relu"))
#         self.add(Dense_V2(n_units=256, input_shape=(512,), activation="relu"))
#         self.add(Dense_V2(n_units=128, input_shape=(256,), activation="relu"))
#         self.add(Dense_V2(n_units=latent_dim, input_shape=(128,), activation="relu"))

#         # self.add(Flatten(input_shape=img_shape))
#         # self.add(Dense_V2(n_units=512, input_shape=(np.prod(img_shape),), activation="leakyrelu", optimizer="adam"))
#         # self.add(Dropout(lowest_value=0.5, input_shape=(512,)))
#         # self.add(Dense_V2(n_units=256, input_shape=(512,), activation="leakyrelu", optimizer="adam"))
#         # self.add(Dropout(lowest_value=0.5, input_shape=(256,)))
#         # self.add(Dense_V2(n_units=1, input_shape=(256,), activation="sigmoid", optimizer="adam"))


#         # # set network
#         # self.add(Flatten(input_shape=img_shape))
#         # self.add(Dense_V2(512, input_shape=(784, ), activation="leakyrelu", optimizer="adam"))
#         # self.add(Dense_V2(512, input_shape=(512, ), activation="leakyrelu", optimizer="adam"))

#         # # remove when using `mu` and `log_var`
#         # self.add(Dense_V2(latent_dim, input_shape=(512, ), activation="leakyrelu", optimizer="adam"))

#         self.mu      = Dense_V2(latent_dim, input_shape=(512, ))
#         self.log_var = Dense_V2(latent_dim, input_shape=(512, ))

#     def reparameterization(self, mu, log_var):
#         epsilon = np.random.normal(mu)
#         return mu + np.exp(0.5 * log_var) * epsilon

#     def forward(self, data):
#         x = self._forward(data)

#         # mu = self.mu.forward(x)
#         # logvar = self.log_var.forward(x)
...
#         # initialize
#         self.encoder = Encoder(self.img_dim, self.latent_dim)
...
#         self.network.layers = np.concatenate((self.decoder.layers, self.encoder.layers))
#         self.network.summary(name="Denoising Autoencoder")

#         # Build the combined model
#         self.combined_1 = Network_V2(loss_name="CrossEntropy")
#         self.combined_1.layers = np.concatenate(
#             (self.decoder.layers, self.discriminator.layers)
#         )
#         self.combined_2 = Network_V2(loss_name="CrossEntropy")
#         self.combined_2.layers = np.concatenate(
#             (self.decoder.layers, self.encoder.layers)
#         )

#         # display network structures
#         self.encoder.summary(name="Encoder")
#         self.decoder.summary(name="Decoder")
#         self.discriminator.summary(name="Discriminator")

#     def build_decoder(self, optimizer="adam", loss_function="CrossEntropy"):
#         model = Network_V2(loss_name="MSE")

#         model.add(Dense_V2(n_units=128, input_shape=(self.latent_dim,), activation="relu"))
#         model.add(Dense_V2(n_units=256, input_shape=(128,), activation="relu"))
#         model.add(Dense_V2(n_units=512, input_shape=(256,), activation="relu"))
#         model.add(Dense_V2(n_units=self.img_dim, input_shape=(512,)))

#         # model = Network_V2(loss_name=loss_function)
        
#         # model.add(Dense_V2(n_units=512, input_shape=(self.latent_dim,), activation="leakyrelu", optimizer=optimizer))
#         # model.add(Dense_V2(n_units=512, input_shape=(512,), activation="leakyrelu", optimizer=optimizer))
#         # model.add(Dense_V2(n_units=np.prod(self.img_shape), input_shape=(512,), activation="tanh", optimizer=optimizer))
...
#         model.add(Dense_V2(512, input_shape=(self.latent_dim, ), activation="leakyrelu", optimizer="adam"))
#         model.add(Dense_V2(256, input_shape=(512, ), activation="leakyrelu", optimizer="adam"))
#         model.add(Dense_V2(1, input_shape=(256, ), activation="sigmoid", optimizer="adam"))
...
#             # gen_image = self.network.predict(rand_image)
#             gen_image = self.decoder.predict(noise)
#             # latent_fake = self.encoder.predict(rand_image)
#             latent_real = np.random.normal(size=(batch_size, self.latent_dim))

#             # Train the discriminator
#             d_loss_real, d_acc_real = self.encoder.train_on_batch(gen_image, valid)
#             d_loss_fake, d_acc_fake = self.encoder.train_on_batch(gen_image, fake)
...
#             # print(noise.shape)
#             # print(valid.shape)
#             g_loss, g_acc = self.combined_2.train_on_batch(noise, valid)
#             # return False
...
#         gen_image = self.network.predict(noise)

#         # Rescale image 0 - 1
#         gen_image = 0.5 * gen_image + 0.5

#         fig, axs = plt.subplots(r, c)
#         plt.suptitle("Variational Network")
#         cnt = 0
#         for i in range(r):
#             for j in range(c):
...
# # sys.path.insert(1, os.getcwd() + "/../../network") 
# # from layers import Dense_V2, Dropout, Reshape, Flatten, BatchNormalization, Network_V2, Activation

# # class Encoder():
# #     def __init__(self, img_shape, latent_dim):
# #         self.model = Network_V2(loss_name="CrossEntropy")
        
# #         self.model.add(Flatten(input_shape=img_shape))
# #         self.model.add(Dense_V2(512, input_shape=(784, ), optimizer="adam"))
# #         self.model.add(Activation("leakyrelu"))
# #         self.model.add(Dense_V2(512, input_shape=(512, ), optimizer="adam"))
# #         self.model.add(Activation("leakyrelu"))
...
# #         self.model.add(Dense_V2(512, input_shape=(latent_dim, ), optimizer="adam"))
# #         self.model.add(Activation("leakyrelu"))
# #         self.model.add(Dense_V2(512, input_shape=(512, ), optimizer="adam"))
# #         self.model.add(Activation("leakyrelu"))
# #         self.model.add(Dense_V2(np.prod(img_shape), input_shape=(512, ), optimizer="adam"))
# #         self.model.add(Activation("tanh"))
# #         self.model.add(Reshape(output_shape=img_shape, input_shape=(np.prod(img_shape), )))

# #     def forward(self, img):
# #         return self.model._forward(img)
...
# #         self.model.add(Dense_V2(512, input_shape=(latent_dim, ), optimizer="adam"))
# #         self.model.add(Activation("leakyrelu"))
# #         self.model.add(Dense_V2(256, input_shape=(512, ), optimizer="adam"))
# #         self.model.add(Activation("leakyrelu"))
# #         self.model.add(Dense_V2(1, input_shape=(256, ), optimizer="adam"))
# #         self.model.add(Activation("sigmoid"))

# #     def forward(self, img):
# #         return self.model._foraward(img)
...
# #         # initialize
# #         self.encoder = Encoder(self.img_shape, self.latent_dim)
# #         self.decoder = Decoder(self.img_shape, self.latent_dim)
# #         self.discriminator = Discriminator(self.img_shape, self.latent_dim)

# #         self.encoder.model.summary(name="ENCODER")
# #         self.decoder.model.summary(name="DECODER")
# #         self.discriminator.model.summary(name="DISCRIMINATOR")
        
# #         # Build the combined model 1
# #         self.combined_1 = Network_V2(loss_name="CrossEntropy")
# #         self.combined_1.layers = np.concatenate(
# #             (self.encoder.model.layers, self.discriminator.model.layers)
# #         )

        
# #         # Build the combined model 2
# #         self.combined_2 = Network_V2(loss_name="MSE")
# #         self.combined_2.layers = np.concatenate(
...
# #             rand_image = X_train[random_index]

# #             latent_fake = self.encoder.forward(rand_image)
# #             latent_real = np.random.normal(size=(batch_size, self.latent_dim))

# #             # Train the discriminator
# #             d_loss_real, d_acc_fake = self.discriminator.model.train_on_batch(latent_real, valid)
# #             d_loss_fake, d_acc_real = self.discriminator.model.train_on_batch(latent_fake, fake)
# #             d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)
# #             d_acc = 0.5 * np.add(d_acc_real, d_acc_fake)
...
# #             # print(valid)
# #             noise = np.random.normal(0, 1, (batch_size, self.latent_dim))
# #             loss_1, acc_1 = self.combined_1.train_on_batch(noise, valid)
# #             # loss_2, acc_2 = self.combined_2.train_on_batch(rand_image, rand_image)
...
# #             data = self.encode(rand_image)
# #             y_pred = self.decoder.predict(data)

# #             g_loss = np.mean(self.decoder.loss_function(valid, y_pred))
# #             loss_grad = self.decoder.loss_function.gradient(valid, y_pred)

# #             loss_grad = self.decoder._backward(loss_grad, y_pred)
# #             loss_grad = self.sampling_backward(loss_grad, y_pred)
# #             loss_grad = self.encoder._backward(loss_grad, y_pred)
...
# #             # g_loss = self.train_on_batch(rand_image, valid)
...
# #     os.makedirs("sample_scratch_output", exist_ok=True)
# #     network = VAE()
# #     network.train(n_epochs=30000, batch_size=32, save_interval=400)
...
# # #     # Optimizers
# # #     coder_parameters = itertools.chain(encoder.parameters(), decoder.parameters())
# # #     optimizer_G = torch.optim.Adam(coder_parameters, lr=lr, betas=(b1, b2))
