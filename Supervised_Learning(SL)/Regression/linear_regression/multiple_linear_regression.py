...
def earlyExit(cost_history, index):
    check = 8

    if index > check:
        start = (index - check)
        mean = cost_history[start : index].mean()
        last = cost_history[index]
...
# loading data
dataSet = pd.read_csv('../_DATA/data_4.csv')
dataSet.head() # To get first n rows from the dataset default value of n is 5

X = dataSet.values[:, 0:2]  # get input values from first two columns
y = dataSet.values[:, 2]    # get output values from last column
m = len(y) # Number of Training Sets

# normalize data
X_norm, mean_values, standard_deviations = normalize_data(X)
...
displayPlot(X_norm, y, theta, cost_history)