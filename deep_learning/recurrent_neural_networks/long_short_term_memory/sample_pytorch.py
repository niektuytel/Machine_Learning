...
training_data = MinMaxScaler().fit_transform(training_set)

def generate_dataset(data, seq_length):
    ...
    return X, y
x, y = generate_dataset(training_data, seq_length)
...
data_predict = MinMaxScaler().inverse_transform(data_predict)
y_data_plot = MinMaxScaler().inverse_transform(y_data_plot)
